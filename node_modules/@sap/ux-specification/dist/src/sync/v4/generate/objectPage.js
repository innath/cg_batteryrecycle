...
function handleCollection(facet, facetKey, appSchema, sections, sectionPrefix, generateParameters, v4Page, level) {
    ...
    if (isCustomSubSectionsSupported) {
        appyCustomSubsections(facet, facetKey, subsections, appSchema, generateParameters, v4Page);
    }
}
/**
...
function generateObjectPageSchemaV4(generateParameters, genericSchema, contextPath, genericMacrosSchema) {
    ...
    if (generateParameters.entityType) {
        ...
        const v4Page = (0, utils_2.findPageV4)(pages, ux_specification_types_1.v4.FE_TEMPLATE_V4_OBJECT_PAGE, generateParameters.entitySet, contextPath, generateParameters.logger);
        if (!v4Page) {
            return appSchema;
        }
        // Add Header
        ...
        // Add sections
        sections[ux_specification_types_1.SchemaTag.isViewNode] = true;
        sections[ux_specification_types_1.SchemaTag.annotationPath] = `/${generateParameters.entityType.fullyQualifiedName}/@${"com.sap.vocabularies.UI.v1.Facets" /* UIAnnotationTerms.Facets */}`;
...
/**
 * Method updates schema for custom subsections.
 * @param {FacetConfig} facet - the given facet from the UI annotations
 * @param {string} facetKey - key of the facet, as used in annotation
 * @param {Definition} subsections - sub sections definition
 * @param {Definition} appSchema - app schema in general
 * @param {GenerateSchemaParameters} generateParameters - list of API input parameters
 * @param {SapUiAppPageV4} v4Page Current page in manifest
 */
function appyCustomSubsections(facet, facetKey, subsections, appSchema, generateParameters, v4Page) {
    const isMergedSections = !containsCollectionFacet(facet);
    const customSubsectionRef = createCustomSubSectionReference(appSchema, facetKey, generateParameters.featureToggles);
    subsections.additionalProperties = {
        $ref: `#/definitions/${customSubsectionRef}`
    };
    (0, utils_1.addCustomSubSectionDefinition)(appSchema, subsections, v4Page, facetKey, customSubsectionRef, isMergedSections, generateParameters.logger);
    const subSectionDefinition = appSchema.definitions[customSubsectionRef];
    if (typeof subSectionDefinition === 'object') {
        (0, common_1.addDescriptiveEnumDefinition)(appSchema, subsections, {
            schema: subSectionDefinition,
        ...
        const relatedFacet = subSectionDefinition.properties.relatedFacet;
        if (isMergedSections && typeof relatedFacet === 'object') {
