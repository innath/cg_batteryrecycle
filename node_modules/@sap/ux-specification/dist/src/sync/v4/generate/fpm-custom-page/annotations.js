...
function applyAnnotationPath(schema, entityTypeResolution) {
    if (entityTypeResolution && entityTypeResolution.metaPath) {
    ...
    'sap.fe.macros.Table': (params) => {
        ...
        if (typeof colDefinition === 'object' && typeof actionDefinition === 'object' && entityTypeResolution) {
            ...
            if (schema.annotationPath) {
                const annotation = entityTypeResolution.metaPath
                    ? (0, common_1.getAnnotationFromMetaPath)(entityTypeResolution.metaPath)
                    : 'LineItem';
                const lineItemAnnotation = entityTypeResolution.entityType?.annotations?.[alias]?.[annotation];
