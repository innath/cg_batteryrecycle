...
async function destinationForServiceBinding(serviceInstanceName, options = {}) {
    if (options.useCache) {
        const fromCache = await destination_cache_1.destinationCache.retrieveDestinationFromCache(options.jwt || (0, destination_from_registration_1.decodedJwtOrZid)().subaccountid, serviceInstanceName, 'tenant');
        if (fromCache) {
            return fromCache;
        }
    }
    ...
    if (options.useCache) {
        ...
        await destination_cache_1.destinationCache.cacheRetrievedDestination(options.jwt || (0, destination_from_registration_1.decodedJwtOrZid)().subaccountid, destWithProxy, 'tenant');
    }
